      ...
      <div className="flex-1 overflow-y-auto">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <button
                ...
                className="flex items-center gap-2 text-xs text-muted-foreground font-mono hover:text-neon-cyan transition-colors"
              >
                {shortAddress}
                <Copy size={10} />
              </button>
              <div className="flex gap-2.5 mt-2.5">
                <span className="text-xs px-2.5 py-1 rounded-full bg-neon-purple/15 text-neon-purple border border-neon-purple/20 font-medium">
                  {t("profile.genesis")}
                </span>
                <span className="text-xs px-2.5 py-1 rounded-full bg-neon-cyan/15 text-neon-cyan border border-neon-cyan/20 font-medium">
                  SBT Holder
                </span>
              </div>
            </div>
        ...
        <div className="mx-4 mt-3 p-3 rounded-xl bg-card/50 border border-border/20">
          <div className="flex items-center justify-between mb-2.5">
            ...
            <span className="text-xs text-muted-foreground font-mono">{(stats?.npPoints ?? 0).toLocaleString()} / {Math.ceil(((stats?.npPoints ?? 0) + 1) / 10000) * 10000} NP</span>
          </div>
          ...
          <p className="text-xs text-muted-foreground mt-2">{t("profile.nextLevel")}</p>
        </div>
        ...
        <div className="px-4 py-4 space-y-4">
          {menuSections.map((section, sIndex) => (
            ...
            >
              <h3 className="text-xs text-muted-foreground font-medium mb-2 px-2">{section.title}</h3>
          ...
          >
            <h3 className="text-xs text-muted-foreground font-medium mb-2 px-2">{t("profile.settings")}</h3>
            <div className="rounded-2xl bg-card/50 border border-border/20 overflow-hidden divide-y divide-border/10">
              {/* Theme Toggle */}
              <button
                onClick={() => toggleTheme?.()}
          ...
          >
            <h3 className="text-xs text-muted-foreground font-medium mb-2 px-2">{t("profile.language") || "Language"}</h3>
          ...
          <div className="text-center pb-4">
            <p className="text-xs text-muted-foreground/40 font-mono">NexusChat v2.1.0</p>
            <p className="text-xs text-muted-foreground/30 mt-0.5">Built for Web3 • Powered by NexusAI</p>
          </div>
        </div>
      </div>
    </div>
