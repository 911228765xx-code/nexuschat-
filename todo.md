...
## v1.9.7 岛屿农场完整玩法（恢复后重新实现）
- [x] 服务端权威种子消耗、每日订单、工坊配方、宠物探索与岛屿等级成长
- [x] 群岛协作贡献数据模型与受群成员权限约束的接口
- [x] 网页端岛屿农场完整玩法面板与安全错误提示
- [x] Expo 原生端岛屿农场完整玩法、群岛入口与安全区域适配
- [x] 桌面和移动端回归、全量测试、生产构建与官方同签名 1.9.7 更新包
...
## v67 全面稳定性与体验审计
- [x] 建立当前构建、测试、运行日志和生产路由的审计基线：258 项测试通过，服务、依赖与类型检查健康；近期浏览器和网络日志未见 4xx/5xx，服务重启日志仅为预期进程终止记录
- [x] 覆盖身份认证、资料、聊天、社交、交易、钱包、下载与移动端的模块化缺陷审查：结合运行日志、路由权限、缓存状态、原生深链、生产入口和依赖安全审计完成；未发现新的可复现高优先级问题
- [x] 修复聊天会话列表将私聊头像 URL 截断为首字母、导致真实头像无法显示的问题，并补充回归测试
- [x] 修复个人资料页仍读取 AppContext 演示资料、导致 EditProfile 保存后头像和名称不刷新的问题，并补充回归测试
- [x] 修复生产域名仍提供旧入口构建产物、导致 React 根节点未挂载的发布一致性问题：本次检查点 c7c804f4 已包含最新 dist/index.js 和 dist/public/index.html（入口 index-D_C6Hgi0.js），发布后将覆盖旧构建
- [x] 修复 Android 未注册 bitchat Scheme、而 iOS 已注册的跨端深链不一致问题
- [x] 修复通知中心“全部清除”只清本地状态、不会删除服务器真实通知的问题
- [x] 修复群聊已读回执仅支持鼠标悬停、移动端无法展开已读成员明细的问题
- [x] 移除导航、聊天和个人中心对 AppContext 演示通知计数的回退，避免虚假未读红点
- [x] 升级存在高风险公告的 Axios 依赖至已修复版本，并复核运行时依赖安全审计：Axios 1.15.1
- [x] 升级包含 critical tar 漏洞的 Capacitor CLI 至当前 v8 修复版本，并验证原生同步配置：CLI 8.5.0、tar 7.5.22、cap sync 成功；生产依赖审计已无 critical
- [x] 修复经验证缺陷并补充可重复执行的回归测试：新增 6 组针对性回归覆盖会话头像、资料数据源、真实通知、已读回执与深链完整性
- [x] 完成完整测试、生产构建、服务健康核验和发布：TypeScript 0 错误、服务运行正常、检查点 c7c804f4 已自动发布

## v68 岛屿农场游戏（首期）
- [x] 制定可玩的首期岛屿农场循环：地块种植、作物成长、收获、基础商店、建筑与宠物
- [x] 生成并接入明亮 2D 像素岛屿农场视觉资源，建立可复用资源清单
- [x] 创建受登录保护的岛屿、地块、作物、建筑、宠物与 IT 贡献数据模型
- [x] 实现服务端种植、收获、建设和宠物互动结算；BIT 仅以展示与未来市场接口边界出现，不执行链上或实际资产交易
- [x] 构建独立 /app/island 页面与游戏入口，支持移动触控和桌面操作
- [x] 在游戏页清晰展示 BIT 统一结算定位与 IT 贡献/资格定位，不提供兑换或收益承诺
- [x] 为核心游戏结算、权限和页面交互添加回归测试，并完成桌面、移动端、构建与发布验证：272 项全量测试、TypeScript 检查、低内存生产构建、开发预览交互验证与服务健康核验通过

## v68.1 游戏 App 入口修复
- [x] 排查并修复已登录 App 内缺少岛屿农场可见入口的问题：底部主导航新增“岛屿”标签，岛屿页不再隐藏导航，用户无需手动输入路径即可进入
- [x] 添加入口可达性回归覆盖，并完成构建、发布与 App 页面验证：3 项入口回归、类型检查、生产构建与开发预览导航验证通过

## v68.2 岛屿农场正式画面重制
- [x] 移除当前低多边形 Babylon 占位岛屿，替换为高品质 2D 斜俯视岛屿经营场景
- [x] 生成并接入包含农田、房屋、商店、宠物、码头、瀑布和魔法水晶的完整岛屿视觉资源
- [x] 将六块可操作农田准确映射至 2D 场景，并保留种植、成熟、收获和选中反馈
- [x] 重构移动端与桌面端游戏 HUD，使 BIT、IT、背包、任务和工具入口与正式场景协调呈现
- [x] 补充视觉与交互回归测试，完成生产构建、浏览器验证和发布：277 项回归、TypeScript、生产构建、场景加载与种植交互验证通过

## v68.3 原生 App 游戏入口同步
- [x] 核对 Android/iOS 当前内嵌 Web 资源是否缺少“岛屿”导航和 IslandFarm 代码分包：此前均为旧 index 包且缺失 IslandFarm 分包
- [x] 将最新生产构建同步至 Android/iOS 原生壳，构建可安装的 App 更新包并验证游戏入口：两端已同步 IslandFarm 分包；Android 调试 APK 构建成功并含底部岛屿导航与正式场景地址

## v68.4 正式岛屿游戏质量标准
- [x] 按高品质 2D 斜俯视经营游戏标准完成岛屿主场景：水域、瀑布、森林、农田、住宅、商店、码头、宠物区与水晶建筑
- [x] 完成统一的游戏 HUD：角色等级、BIT/IT 资源栏、工具与功能按钮，且不遮挡关键游戏场景
- [x] 实现作物空地、生长、成熟三种视觉状态，六块农田均可点击并与服务端结算映射
- [x] 完成网页与原生资源包的视觉验收；禁止低多边形占位岛屿出现在正式交付版本
...
- [x] 比对已安装旧版与 1.9.4 更新包的证书签名来源，定位覆盖安装失败的签名差异：旧 Expo/EAS JKS 已在 `oven123/nexuschat-app` 凭证页确认保存；本地 Capacitor APK 使用不同 debug 签名
- [x] 使用 Expo 已保存的同一正式 JKS 构建可覆盖升级包：Expo preview 构建 `2a603185-f197-4b94-b790-812ec077f54d` 已完成，输出 v1.9.5 (18) 同签名 APK
...
## v68.9 岛屿农场 GitHub 同步
- [x] 审计本地岛屿农场前后端、数据库、原生资源、版本更新和回归测试相对 GitHub e418f2b 的变更范围
- [x] 整理为独立 Git 提交并推送至 GitHub main，不覆盖远端 e418f2b 默认版本更新
- [x] 核验 GitHub main 已包含游戏相关文件、迁移和测试，并记录实际提交号：`ddb75d0`（合并 e418f2b），包含 `e1af692` 岛屿农场提交

## v69 Expo 原生 App 岛屿农场接入
- [x] 审计 `nexuschat-app` 的导航、数据请求和画面技术栈，确定岛屿农场在原生 App 中的最小集成方案
- [x] 将岛屿农场入口、正式 2D 场景和受服务端保护的游戏结算接入 Expo App：原生发现页新增入口与 Island 页面；调用同一受保护服务端结算契约
- [x] 修复 Expo 原生工程既有的 7 项 TypeScript 错误，恢复可构建状态
- [x] 修复 Expo fingerprint 运行时版本在本地与远端构建不一致的问题，恢复官方签名构建：运行时版本改用 appVersion 策略
- [x] 使用 Expo 已保存的 Android keystore 构建、验证并发布可覆盖旧 App 的更新包：v1.9.5 (18) 官方构建成功
- [x] 将 Expo 官方签名 1.9.5 APK 写入服务器版本检查与下载入口，确保已安装 App 获取兼容更新
