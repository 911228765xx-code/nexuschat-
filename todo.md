...
- [x] 后端：Research 报告生成接口已注入 CoinGecko 实时市场数据（价格/市值/涨跌幅/ATH/供应量/情绪等）到 LLM 系统提示词
- [x] 后端：Deep 模式增强——并行获取全球市场数据(BTC主导率/总市值) + 恐惧贪婪指数 + BTC价格 + 30天价格趋势(SMA7/SMA14/区间位置)；Quick 模式轻量级获取恐惧贪婪指数
- [x] 147 个测试全部通过，0 TS 错误

## v45b 投研报告精简 + 可视化优化

- [x] Quick/Deep 提示词精简：Quick 250-350字、Deep 600-800字，系统提示词强制禁止套话废话，每句必须含具体数字
...
## v49 策略编辑/删除 + Market i18n

- [x] 后端：copyTrading 路由增加 deleteStrategy mutation（只允许删除自己的策略）
- [x] 前端：编辑按鈕预填 newStrategy 表单所有字段，模态标题切换为“编辑策略”，upsertStrategy 传 id 实现更新
