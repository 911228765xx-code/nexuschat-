...
- [ ] 消息长按菜单（复制/删除/回复，移动端长按+桌面端悬停）
- [ ] 退出群组功能（侧边栏设置入口）
- [ ] 群组设置页（群主可改名/改描述/改头像）
- [ ] 图片消息渲染（显示缩略图而非文字）
- [ ] 消息搜索界面优化（替换浏览器prompt）
- [ ] 群主/管理员身份标识（消息气泡旁显示角色标签）
- [ ] 双端模拟测试（电脑端 + 手机端）
...
## v64 生产环境问题排查
- [ ] 排查生产环境搜索框排版问题（是否未使用最新构建）
- [ ] 排查生产环境登录黑屏问题（是否与开发环境修复不一致）
- [ ] 确认生产环境使用的是最新发布的版本
...
## v57 发布动态弹窗修复 + BscScan 交易记录
- [ ] 「发布动态」弹窗用 createPortal 渲染到 document.body，修复 overflow-hidden 父容器问题
- [ ] 配置 BscScan API Key，getTransactions 返回真实历史交易记录
- [ ] 移动端模拟测试验证效果

## v57b Moralis API 真实交易记录
- [x] 用户注册 Moralis 并提供 API Key
- [x] 服务端 getTransactions 改用 Moralis Web3 API 获取 BSC 真实交易历史
- [x] 160 个测试全部通过，Moralis API Key 验证成功
- [x] 移动端模拟测试验证（登录页显示正常，连接钱包后可看交易记录）
